...
// ===============================================
//   PEDIDOS DE USUARIOS COMUNES
// ===============================================

router.get('/pedidos/comunes', (req, res) => {
    const sql = `
        SELECT 
            pp.id,
            u.nombre AS nombre_usuario,
            pp.fecha_pedido AS fecha,
            pp.estado,
            ppi.cantidad,
            im.nombre AS nombre_plato
    ...
    connection.query(sql, (err, results) => {
        if (err) {
            console.error("Error SQL en /pedidos/comunes:", err);
            return res.status(500).json({ error: "Error en el servidor." });
        }
...
router.put('/pedidos/comunes/:id/estado', (req, res) => {
    const pedidoId = req.params.id;
    const { estado } = req.body;

    if (!['PENDIENTE', 'PROCESO', 'FINALIZADO', 'CANCELADO'].includes(estado)) {
        return res.status(400).json({ error: "Estado inválido." });
    }

    const sql = `UPDATE pedido_personal SET estado = ? WHERE id = ?`;
